...
export default function Listings(){
 ...
 async function load(){
  ...
  const [{data,error},{data:messages,error:inquiryError}]=await Promise.all([
   s.from('fmfh_properties').select('id,title,status,monthly_rent,currency,city').eq('owner_id',user.id).order('created_at',{ascending:false}),
 ...
 return <main className="hero"><a href="/">← Home</a><div className="eyebrow" style={{marginTop:28}}>LANDLORD</div><h1>Your listings.</h1>
  ...
  <section style={{display:'grid',gap:14,marginTop:32}}><h2>Your properties</h2>{items.length===0&&<p>No listings yet.</p>}{items.map(x=><article className="card" key={x.id}><div className="eyebrow">{x.status}</div><h3>{x.title}</h3><p>{x.city}</p><strong>{x.currency} {Number(x.monthly_rent).toLocaleString()}/month</strong>{x.status==='draft'&&<p><button className="cta" type="button" onClick={()=>changeStatus(x.id,'published')}>Publish listing</button></p>}{x.status==='published'&&<p><a href={`/property/${x.id}`}>View public page</a> · <button type="button" onClick={()=>changeStatus(x.id,'rented')}>Mark as rented</button></p>}</article>)}</section>
