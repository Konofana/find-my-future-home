...
export default function Listings(){
 ...
 async function publish(id:string){const {data:{user}}=await s.auth.getUser();if(!user){setMessage('Log in first.');return}
 const {error}=await s.from('fmfh_properties').update({status:'published',published_at:new Date().toISOString()}).eq('id',id).eq('owner_id',user.id).eq('status','draft')
 if(error){setMessage(error.message);return}setMessage('Listing published.');await load()}
 ...
 {message&&<p role="status">{message} {message.includes('Log in')&&<a href="/account">Go to account</a>}</p>}<div style={{display:'grid',gap:14,marginTop:24}}>{items.map(x=><article className="card" key={x.id}><div className="eyebrow">{x.status}</div><h2>{x.title}</h2><p>{x.city}</p><strong>{x.currency} {Number(x.monthly_rent).toLocaleString()}/month</strong>{x.status==='draft'&&<p><button className="cta" type="button" onClick={()=>publish(x.id)}>Publish listing</button></p>}</article>)}</div></main>
}
